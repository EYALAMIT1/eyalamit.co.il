	...
	public function render( array $attributes, $content ) {
		...
		foreach( $this->get_marker_attribute_array( $attributes ) as $marker ) {
			if ( $marker[ 'markerSource' ] === 'address' ) {
				$address_data = Toolset_Addon_Maps_Common::get_coordinates( $marker[ 'markerAddress' ] );
				if ( is_array( $address_data ) ) {
					$marker[ 'markerLat' ] = $address_data['lat'];
					$marker[ 'markerLon' ] = $address_data['lon'];
				}
				...
				// and process it.
				$marker[ 'markerLat' ] = 'geo';
				$marker[ 'markerLon' ] = $marker[ 'currentVisitorLocationRenderTime' ];
			} elseif ( $marker[ 'markerSource' ] === 'dynamic') {
				$post_id = get_the_ID();
				$address = do_shortcode(
					"[tb-dynamic provider='{$marker[ 'markerDynamicAddress' ]->provider}' post='{$post_id}' "
					."source='{$marker[ 'markerDynamicAddress' ]->source}' "
					."field='{$marker[ 'markerDynamicAddress' ]->field}' force-string='first']"
				);
			...
			$output .= Toolset_Addon_Maps_Common::render_marker(
				$map_id,
				array(
					'id'			=> $marker[ 'markerId' ],
					'title'			=> $marker[ 'markerTitle' ],
					'lat'			=> $marker[ 'markerLat' ],
					'lon'			=> $marker[ 'markerLon' ],
