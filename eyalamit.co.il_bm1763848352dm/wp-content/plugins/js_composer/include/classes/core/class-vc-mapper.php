...
class Vc_Mapper {
	...
	protected function callActivities() {
		...
		foreach ( $this->init_activity as $key => $activity_data ) {
			list( $object, $method, $params ) = $activity_data;
			if ( 'mapper' === $object ) {
				switch ( $method ) {
					case 'map':
	...
	public function callElementActivities( $tag ) {
		do_action( 'vc_mapper_call_activities_before' );
		if ( isset( $this->element_activities[ $tag ] ) ) {
			foreach ( $this->element_activities[ $tag ] as $key => $activity_data ) {
				list( $method, $params ) = $activity_data;
				switch ( $method ) {
					case 'drop_param':
						WPBMap::dropParam( $params['name'], $params['attribute_name'] );
						break;
