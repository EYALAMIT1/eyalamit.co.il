...
class Envira_Albums_Shortcode {
    ...
    public function __construct() {
        ...
        wp_register_style( $this->galleryBase->plugin_slug . '-lazyload', plugins_url( 'assets/css/responsivelyLazy.css', $this->galleryBase->file ), array(), $this->base->version );
    ...
    public function maybe_add_back_link_prepend( $gallery, $data ) {
        ...
        if ( ( !empty( $referer_url_parts ) && $referer_url_parts[0] != $slug ) || ( isset( $_REQUEST['album_id'] ) ) ) {
            ...
            if ( $maybe_album_page[0]->post_type == 'page' || $maybe_album_page[0]->post_type == 'post' ) {
                ...
                if ( !$this->get_config( 'back_location', $album_data ) || $this->get_config( 'back_location', $album_data ) == 'above' || $this->get_config( 'back_location', $album_data ) == 'above-below' ) {
    ...
    public function maybe_add_back_link_append( $gallery, $data ) {
        ...
        if ( ( !empty( $referer_url_parts ) && $referer_url_parts[0] != $slug ) || ( isset( $_REQUEST['album_id'] ) ) ) {
            ...
            if ( $maybe_album_page[0]->post_type == 'page' || $maybe_album_page[0]->post_type == 'post' ) {
                ...
                if ( $this->get_config( 'back_location', $album_data ) == 'below' || $this->get_config( 'back_location', $album_data ) == 'above-below' ) {
                        // Append Back to Album Button
                        $gallery = $gallery . '<a href="' . get_permalink( $album_id ) . '" title="' . $this->get_config( 'back_label', $album_data ) . '" class="envira-back-link">' . $this->get_config( 'back_label', $album_data ) . '</a>'; 
                }
        ...
        $output = '<div id="envira-gallery-item-' . sanitize_html_class( $id ) . '" class="' . $this->get_gallery_item_classes( $item, $i, $data ) . '" style="padding-left: ' . $padding . 'px; padding-bottom: ' . $this->get_config( 'margin', $data ) . 'px; padding-right: ' . $padding . 'px;" ' . apply_filters( 'envira_albums_output_item_attr', '', $id, $item, $data, $i ) . '>';
            ...
            if ( isset( $data['config']['gallery_description_display'] ) && $data['config']['gallery_description_display'] == 'display-above' && (int) $data['config']['columns'] !== 0 && isset( $item['id'] ) ) {
                ...
                if ( isset( $gallery_data['config']['description'] ) && $gallery_data['config']['description'] ) {
                    ...
                    $gallery_description = wp_kses( $gallery_data['config']['description'], $allowed_html_tags );
            ...
            if ( isset( $data['config']['display_titles'] ) && $data['config']['display_titles'] === 'above' && (int) $data['config']['columns'] !== 0 ) {
                ...
                if ( ! empty( $item['title'] ) ) {
                    $output .= '<div class="envira-album-title">' . htmlspecialchars_decode( $item['title'] ) . '</div>';
                }
            ...
            if ( $create_link ) {
                ...
                $output .= 'href="'. get_permalink( $id ) .'" class="envira-album-gallery-' . $id . ' envira-gallery-link" title="' . strip_tags( htmlspecialchars_decode( $item['title'] ) ) . '" ' . apply_filters( 'envira_gallery_output_link_attr', '', $id, $item, $data, $i ) . '>';
            ...
            if ( $this->get_config( 'columns', $data ) == 0 ) {
                ...
                $output_item = '<img id="envira-gallery-image-' . sanitize_html_class( $id ) . '" class="envira-gallery-image envira-gallery-image-' . $i . $gallery_theme . ' '.$envira_lazy_load.'" src="' . esc_url( $imagesrc ) . '"' . ( $this->get_config( 'dimensions', $data ) ? ' width="' . $this->get_config( 'crop_width', $data ) . '" height="' . $this->get_config( 'crop_height', $data ) . '"' : ' height="'.$output_height.'" width="'.$output_width.'"' ) . ' data-envira-width="'.$output_width.'" data-envira-height="'.$output_height.'" data-envira-src="' . esc_url( $output_src ) . '" data-envira-caption="' . htmlentities($item_caption) . '" data-envira-item-id="' . $id . '" data-envira-album-id="' . $data['id'] . '" data-envira-gallery-id="' . sanitize_html_class( $id ) . '" alt="' . esc_attr( $item['alt'] ) . '" title="' . strip_tags( htmlspecialchars_decode( $item['title'] ) ) . '" ' . apply_filters( 'envira_albums_output_image_attr', '', $item['cover_image_id'], $item, $data, $i ) . ' srcset="' . ( ( $envira_lazy_load ) ? 'data:image/gif;base64,R0lGODlhAQABAIAAAP///////yH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==' : esc_url( $image_src_retina ) . ' 2x' ) . '" data-safe-src="'. ( ( $envira_lazy_load ) ? 'data:image/gif;base64,R0lGODlhAQABAIAAAP///////yH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==' : esc_url( $output_src ) ) . '" />';
                    ...
                    foreach ( $data['galleryIDs'] as $id ) {
                        ...
                        if ( isset( $gallery['gallery'] ) ) {
                            foreach ( $gallery['gallery'] as $image_id => $image ) {
                                ...
                                if ( !isset( $image["thumb"] ) || empty($image["thumb"]) ) { 
                                    $image['thumb'] = envira_resize_image( $image['src'], $gallery['config']['thumbnails_width'], $gallery['config']['thumbnails_height'], true ); 
                                }
