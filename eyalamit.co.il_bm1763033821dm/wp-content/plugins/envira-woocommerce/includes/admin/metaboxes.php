...
class Envira_WooCommerce_Metaboxes {
	...
	function woocommerce_tab( $post ) {
		...
		if ( !class_exists( 'WooCommerce' )): ?>
			    <div class="envira-external-req">
					...
					<p><a href="<?php echo admin_url('plugin-install.php?tab=search&s=woocommerce'); ?>" class="button button-primary"><?php esc_html_e( 'Click here to download WooCommerce', 'envira-woocommerce' ); ?></a></p>
			    </div>
			<?php else:		
		...
		if ( 'envira' == $post_type ) {
			...
			<p class="envira-intro">
				<?php _e( 'WooCommerce Gallery Options.', 'envira-woocommerce' ); ?>
				<small>
                    <?php _e( 'The settings below adjust the WooCommerce options for the gallery.', 'envira-woocommerce' ); ?>
                    <br/>
		...
		?>
		<p class="envira-intro">
			<?php _e( 'WooCommerce Lightbox Options.', 'envira-woocommerce' ); ?>
		</p>
		<table class="form-table">
			<tbody>
				<tr id="envira-config-woocommerce-box">
    ...
    public function mobile_screen( $post ) {
        ...
        <tr id="envira-config-woo-galleries-mobile-box">
            <th scope="row">
                <label for="envira-config-woo-galleries-mobile"><?php _e( 'Disable WooCommerce In Galleries On Mobile?', 'envira-zoom' ); ?></label>
            </th>
            <td>
                <input id="envira-config-woo-galleries-mobile" type="checkbox" name="<?php echo $key; ?>[mobile_disable_woo_galleries]" value="1" <?php checked( $instance->get_config( 'mobile_disable_woo_galleries', $instance->get_config_default( 'mobile_disable_woo_galleries' ) ), 1 ); ?> />
                <span class="description"><?php _e( 'If enabled, no WooCommerce functionality or buttons will be displayed in galleries on mobile.', 'envira-zoom' ); ?></span>
            </td>
        </tr>
        <?php
