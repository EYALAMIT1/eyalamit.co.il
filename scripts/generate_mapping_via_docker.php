<?php
/**
 * Generate Site Mapping via Docker
 * Script that creates the mapping by connecting to database through Docker
 */
...
// Execute the mapping script inside Docker
...
try {
    ...
    $filename = "/var/www/html/docs/sitemap/COMPREHENSIVE-SITE-MAPPING-{$timestamp}.json";
...
// Save the script to a temporary file and execute it in Docker
$temp_script = tempnam(sys_get_temp_dir(), 'mapping_') . '.php';
file_put_contents($temp_script, $script_content);

// Execute the script in Docker
$command = "cd /Users/nimrod/Documents/Eyal\\ Amit/eyalamit.co.il && docker compose exec -T wordpress php /dev/stdin < {$temp_script}";
...
if ($return_code === 0) {
    ...
    echo "❌ Failed to execute mapping script\n";
    echo "Output: " . implode("\n", $output) . "\n";
}