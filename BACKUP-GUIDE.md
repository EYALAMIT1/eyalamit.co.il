...
```powershell
.\backup-site.ps1                # שימוש ב-env.local כברירת מחדל
.\backup-site.ps1 -EnvFile .env  # טעינת קובץ ENV אחר
.\backup-site.ps1 -SkipFiles     # לדוגמה: גיבוי DB בלבד
```

**מה עוד נוסף בגרסה החדשה:**
- ✅ קורא אוטומטית את ההגדרות מתוך `.env.local` / `.env`
- ✅ מייצר גם `manifest.json` עם SHA256 + תיעוד Git head
- ✅ יוצר עותק `backups/latest/` שמצביע לגיבוי האחרון
- ✅ מאפשר דילוג על DB או קבצים לפי הצורך

**איפה נשמר הגיבוי?**
```
backups/
  ├── backup_2025-11-24_12-30-00/
  │   ├── database_backup.sql
  │   ├── files_backup.zip
  │   └── manifest.json
  └── latest/   # העתק של הגיבוי העדכני ביותר
```
...
**דוגמה:**
```bash
docker exec new-website-ai-nov-2025-take-2-db-1 mysqldump -ueyal -peyalpass eyal_local > backup_20250115.sql
```
...
Compress-Archive -Path ".\eyalamit.co.il_bm1763848352dm\*" -DestinationPath "backup_files_$(Get-Date -Format 'yyyyMMdd_HHmmss').zip"
```
...
**דוגמה:**
```bash
docker exec -i new-website-ai-nov-2025-take-2-db-1 mysql -ueyal -peyalpass eyal_local < backups/backup_2025-01-15_10-30-00/database_backup.sql
```
