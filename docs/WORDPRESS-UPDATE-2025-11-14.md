...
```bash
docker exec newwebsiteainov2025-wordpress-1 wp plugin deactivate disable-wordpress-updates --allow-root --path=/var/www/html
```
...
```bash
# עדכון ל-5.3
docker exec newwebsiteainov2025-wordpress-1 wp core update --version=5.3 --allow-root --path=/var/www/html
docker exec newwebsiteainov2025-wordpress-1 wp core update-db --allow-root --path=/var/www/html

# עדכון ל-5.4
docker exec newwebsiteainov2025-wordpress-1 wp core update --version=5.4 --allow-root --path=/var/www/html
docker exec newwebsiteainov2025-wordpress-1 wp core update-db --allow-root --path=/var/www/html

# עדכון ל-5.5
docker exec newwebsiteainov2025-wordpress-1 wp core update --version=5.5 --allow-root --path=/var/www/html
docker exec newwebsiteainov2025-wordpress-1 wp core update-db --allow-root --path=/var/www/html

# עדכון ישיר ל-6.8.3
docker exec newwebsiteainov2025-wordpress-1 wp core update --allow-root --path=/var/www/html
docker exec newwebsiteainov2025-wordpress-1 wp core update-db --allow-root --path=/var/www/html
```
...
### קבצים ששונו
1. `eyalamit.co.il_bm1763033821dm/wp-config.php` - הוספת הגדרות זיכרון
...
### רשימת פלאגינים
```bash
docker exec newwebsiteainov2025-wordpress-1 wp plugin list --allow-root --path=/var/www/html
```

### עדכון פלאגין ספציפי
```bash
docker exec newwebsiteainov2025-wordpress-1 wp plugin update [plugin-name] --allow-root --path=/var/www/html
```
