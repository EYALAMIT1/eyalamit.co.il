	...
	$( document ).on( 'click', '.envira-social-buttons a', function( e ) {
		...
		if ( url == '#' ) { /* LIGHTBOX / MODAL */
			...
			var link_source     	= $('.envirabox-wrap'),
				...
				if (typeof link_source.attr('data-envira-album-id') !== "undefined") {
					var album_id	= link_source.data('envira-album-id');
					...
					// If WP_DEBUG enabled, output error details
					if ( envira_social.debug ) {
						console.log ('not album_id detected.');
					}
				}
				...
				link			= ( hash.length > 0 ) ? base_link + '?envira_album_id=' + album_id + '&envira_social_gallery_id=' + gallery_id + '&envira_social_gallery_item_id=' + gallery_item_id + '&rand=' + rand + '&' + envira_permalink + hash: base_link + page_string + '?envira_album_id=' + album_id + '&envira_social_gallery_id=' + gallery_id + '&envira_social_gallery_item_id=' + gallery_item_id + '&rand=' + rand + '&' + envira_permalink;
